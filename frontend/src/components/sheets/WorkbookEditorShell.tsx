...
import SheetHistoryPanel from "@/components/sheets/SheetHistoryPanel";
import {
  WhatsAppSendDialog,
...
const UniverSheetEditor = dynamic(
  () => import("@/components/spreadsheet/UniverSheetEditor"),
  {
    ssr: false,
  },
);
...
type SheetStateAction =
  "lock" | "unlock" | "archive" | "unarchive" | "hide" | "unhide";
  ...
  try {
    const value = Number(
      window.localStorage.getItem(
        lastViewedSheetStorageKey(userId, workbookId),
      ),
    );
...
function rememberLastViewedSheet(
  userId: number,
  workbookId: string,
  sheetId: number,
) {
...
export default function WorkbookEditorShell({
  workbookId,
  requestedSheetId,
}: Props) {
  const router = useRouter();
  const { workbook, loading, error, refresh, refreshSilently } =
  ...
  const [optimisticEditableSheetId, setOptimisticEditableSheetId] = useState<
    number | null
  >(null);
  ...
  const [sidebarDragImportUploading, setSidebarDragImportUploading] =
    useState(false);
  const [sidebarDragImportProgress, setSidebarDragImportProgress] = useState(0);
  const [sheetContextMenu, setSheetContextMenu] =
    useState<SheetContextMenuState | null>(null);
  ...
  const canManageWorkbook = Boolean(
    currentUser &&
    (workbook?.can_manage ??
      (isAdmin(currentUser) || currentUser.id === workbook?.owner_id)),
  );
  const compactSidebar = !isMobileLayout && sidebarCollapsed;

  const handleSidebarDroppedXlsxImport = useCallback(
    async (file: File) => {
  ...
  const handleSidebarDragEnter = useCallback(
    (event: React.DragEvent<HTMLDivElement>) => {
      ...
      const hasFile = Array.from(event.dataTransfer.types || []).includes(
        "Files",
      );
      if (!hasFile) return;
  ...
  const handleSidebarDragLeave = useCallback(
    (event: React.DragEvent<HTMLDivElement>) => {
      ...
      if (!hasFile) return;
      event.preventDefault();
      sidebarDragDepthRef.current = Math.max(
        0,
        sidebarDragDepthRef.current - 1,
      );
  ...
  const handleSidebarDrop = useCallback(
    ...
    [
      canManageWorkbook,
      handleSidebarDroppedXlsxImport,
  ...
  useEffect(() => {
    ...
    if (requestedSheetId === null || !sheetIds.includes(requestedSheetId)) {
      const rememberedSheetId = readLastViewedSheetId(
        currentUser.id,
        workbookId,
      );
      const targetSheetId =
        rememberedSheetId && sheetIds.includes(rememberedSheetId)
          ? rememberedSheetId
          : sheetIds[0];
  ...
  const handleDeleteSheet = async (targetSheet: Sheet | null = activeSheet) => {
    ...
    if (
      !window.confirm(
        `确定要删除工作表「${targetSheet.name || "未命名工作表"}」吗？`,
      )
    )
      return;
    ...
    const fallbackSheetId =
      sheets[targetIndex + 1]?.id ?? sheets[targetIndex - 1]?.id ?? null;
  ...
  const handleDuplicateSheet = async (targetSheet: Sheet) => {
    ...
    try {
      ...
      setSheetActionError(
        err instanceof Error ? err.message : "复制工作表失败，请稍后再试。",
      );
    } finally {
      setSheetActionLoading(false);
    }
  ...
  const handleUpdateSheetState = async (
    targetSheet: Sheet,
  ...
  const handleBatchUpdateSheetState = async (action: SheetStateAction) => {
    if (!isAdminUser || selectedSheets.length === 0 || sheetActionLoading)
      return;
    setSheetActionError("");
    setSheetActionLoading(true);
    setSheetContextMenu(null);
    try {
      const results = await Promise.all(
        selectedSheets.map((sheet) =>
          api.put(`/sheets/${sheet.id}/state`, { action }),
        ),
      );
      const failure = results.find((result) => result.code !== 0);
      if (failure) {
        setSheetActionError(
          failure.message || "部分工作表状态更新失败，请稍后再试。",
        );
      }
      await refresh();
    } catch (err) {
  ...
  const handleBatchDeleteSheets = async () => {
    if (!canManageWorkbook || selectedSheets.length === 0 || sheetActionLoading)
      return;
    const selectedNames = selectedSheets
      .slice(0, 3)
      .map((sheet) => `「${sheet.name || "未命名工作表"}」`)
      .join("、");
    const suffix =
      selectedSheets.length > 3 ? ` 等 ${selectedSheets.length} 个工作表` : "";
    if (
      !window.confirm(`确定删除 ${selectedNames}${suffix} 吗？此操作不可撤销。`)
    )
      return;
    ...
    const fallbackSheet =
      sheets.find((sheet) => !selectedIdSet.has(sheet.id)) || null;
    try {
      const results = await Promise.all(
        selectedSheets.map((sheet) => api.delete(`/sheets/${sheet.id}`)),
      );
      const failure = results.find((result) => result.code !== 0);
      if (failure) {
        setSheetActionError(
          failure.message || "部分工作表删除失败，请刷新后重试。",
        );
      }
      await refresh();
      setSelectedSheetIds([]);
      if (activeSheet && selectedIdSet.has(activeSheet.id)) {
        router.replace(
          fallbackSheet
            ? `/sheets/${workbookId}/${fallbackSheet.id}`
            : `/sheets/${workbookId}`,
        );
      }
    } catch (err) {
  ...
  const handleSheetCardClickCapture = (
  ...
  const openSheetContextMenu = (
    ...
    const menuWidth = 240;
    const menuHeight =
      modifierPressed && nextSelectedIds.length > 1
        ? 420
        : canManageWorkbook
          ? 324
          : 80;
  ...
  if (error || !workbook) {
    return (
      <AuthGuard>
        <div className="flex min-h-screen items-center justify-center bg-slate-50">
          <div className="text-center">
            <div className="mb-2 text-sm font-semibold uppercase tracking-widest text-rose-500">
              Oops
            </div>
            <div className="text-lg font-semibold text-slate-900">
              {error || "工作簿未找到"}
  ...
  return (
    <AuthGuard>
      <div
        ref={rootRef}
        className={`flex flex-col bg-slate-50 ${fullscreen ? "fixed inset-0 z-50" : "h-screen"}`}
      >
        <div className="flex min-h-[56px] items-center justify-between gap-2 border-b border-slate-200 bg-white px-2 py-2 sm:min-h-[68px] sm:px-4 sm:py-3">
          <div className="flex min-w-0 items-center gap-1.5 sm:gap-3">
            <button
              ...
              onClick={() =>
                isMobileLayout
                  ? setMobileSheetPanelOpen((current) => !current)
            ...
            >
              {(isMobileLayout ? !mobileSheetPanelOpen : compactSidebar) ? (
            ...
            <div ref={addSheetPopoverRef} className="relative shrink-0">
              ...
              >
                <Plus
                  className={`h-4 w-4 transition-transform ${addingSheet ? "rotate-45" : ""}`}
                />
              </button>
              {addingSheet && (
                <div
                  role="dialog"
                  aria-label="新建工作表"
                  className="absolute -left-12 top-11 z-[120] w-[min(18rem,calc(100vw-2rem))] rounded-lg border border-slate-200 bg-white p-3 shadow-2xl sm:left-0"
                >
                  <div className="mb-2 flex items-center justify-between gap-3">
                    ...
                    <button
                      ...
                      className="ui-tooltip inline-flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700"
                      title="取消新建工作表"
                      aria-label="取消新建工作表"
                      data-tooltip="取消"
                    >
                  ...
                  <button
                    ...
                    className="mt-2 inline-flex h-9 w-full items-center justify-center gap-2 rounded-lg bg-slate-900 px-3 text-sm font-semibold text-white hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50"
                    title="创建工作表"
                  >
            ...
            <button
              type="button"
              onClick={() => router.push(consumeReturnTarget("/"))}
              className="inline-flex h-9 items-center gap-1.5 rounded-lg px-2 text-sm leading-none text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 sm:px-2.5"
            >
            ...
            <div className="flex min-w-0 flex-col justify-center">
              <div className="hidden text-[11px] font-semibold uppercase leading-4 tracking-[0.28em] text-sky-600 sm:block">
                Workbook
              </div>
              <h1 className="max-w-[110px] truncate text-sm font-semibold leading-5 text-slate-900 sm:max-w-[320px]">
                {workbook.name}
              </h1>
              {activeSheet && (
          ...
          <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
            {activeSheet && (
              <button
                ...
                className="ui-tooltip inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 text-slate-500 transition hover:bg-slate-100 hover:text-slate-900"
                title="版本与操作历史"
                aria-label="打开版本与操作历史"
                data-tooltip="版本与操作历史"
              >
              ...
              <button
                ...
                onClick={() =>
                  setWhatsAppResource({
                    ...
                    title: `${workbook.name} / ${activeSheet.name}`,
                    defaultContent: `工作表：${workbook.name} / ${activeSheet.name}`,
                  })
                }
                className="ui-tooltip inline-flex h-9 w-9 items-center justify-center rounded-lg border border-emerald-200 text-emerald-600 transition hover:bg-emerald-50"
                title="发送当前工作表到 WhatsApp"
                aria-label="发送当前工作表到 WhatsApp"
            ...
            {canManageWorkbook && activeSheet && (
              <button
                type="button"
                onClick={() =>
                  void handleUpdateWorkbookState(
                    workbook.is_public ? "unpublish" : "publish",
                  )
                }
            ...
            {isAdminUser && activeSheet && (
              <button
                ...
                onClick={() =>
                  void handleUpdateWorkbookState(
                    workbook.is_locked ? "unlock" : "lock",
                  )
                }
                disabled={sheetActionLoading}
            ...
            {isAdminUser && activeSheet && (
              <button
                ...
                onClick={() =>
                  void handleUpdateWorkbookState(
                    workbook.is_hidden ? "unhide" : "hide",
                  )
                }
                disabled={sheetActionLoading}
                className="hidden items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 transition hover:bg-slate-50 hover:text-slate-900 disabled:cursor-not-allowed disabled:opacity-50 md:inline-flex"
        ...
        {activeSheet &&
          (activeSheet.is_locked || activeSheet.is_archived) &&
          !isAdminUser && (
            <div className="border-b border-amber-200 bg-amber-50 px-4 py-2 text-sm text-amber-800">
              {activeSheet.is_archived
                ? "当前工作表已归档，仅管理员可以修改。"
                : "当前工作表已锁定，仅管理员可以修改或解除锁定。"}
            </div>
          )}
        ...
        <div className="relative flex min-h-0 flex-1 overflow-hidden">
          {isMobileLayout && mobileSheetPanelOpen && (
            <button
              type="button"
              aria-label="关闭工作表目录"
              onClick={() => setMobileSheetPanelOpen(false)}
              className="absolute inset-0 z-30 bg-slate-950/25"
            />
          )}
          <aside
            ...
            style={
              isMobileLayout
                ? {
                    transform: mobileSheetPanelOpen
                      ? "translateX(0)"
                      : "translateX(-100%)",
                  }
                : undefined
              ...
              <div className="border-b border-slate-200 px-4 py-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setSheetSearchQuery(event.target.value)
                    }
                    placeholder="搜索工作表名称..."
                ...
                <div className="mt-3 flex gap-2">
                  <select
                    value={sheetSortBy}
                    onChange={(event) => {
                      const nextSort = event.target.value as SheetSortBy;
                      setSheetSortBy(nextSort);
                      if (nextSort === "workflow") setSheetSortOrder("asc");
                    }}
                  ...
                  >
                    <option value="workflow">按流程排序</option>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setSheetSortOrder(event.target.value as SheetSortOrder)
                    }
                ...
                {selectedSheetIds.length > 0 && (
                  <div className="mt-3 flex items-center justify-between gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-800">
                    ...
                    <button
                      ...
                      className="ui-tooltip inline-flex h-7 w-7 items-center justify-center rounded-lg text-amber-700 hover:bg-white"
                      title="清除工作表选择"
                      aria-label="清除工作表选择"
                      data-tooltip="清除选择"
                      data-tooltip-side="left"
                ...
                {isMobileLayout && activeSheet && (
                  <button
                    ...
                    className="mt-3 inline-flex h-9 w-full items-center justify-center gap-2 rounded-lg border border-rose-200 bg-white text-xs font-semibold text-rose-600 transition hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-40"
                  >
            ...
            >
                      ...
                      <div className="mt-4">
                        ...
                        <div className="mt-2 h-2 overflow-hidden rounded-full bg-sky-100">
                          <div
                            ...
                            style={{ width: `${sidebarDragImportProgress}%` }}
                          />
                        </div>
                      </div>
                    )}
              ...
              >
                {visibleSheets.map((sheet) => {
                  const sheetLabel =
                    sheet.name?.trim() ||
                    `工作表 ${sheets.findIndex((item) => item.id === sheet.id) + 1}`;
                  ...
                  const canRenameThisSheet =
                    canManageWorkbook && !compactSidebar;

                  return (
                    <div
                      key={sheet.id}
                      onClickCapture={(event) =>
                        handleSheetCardClickCapture(event, sheet)
                      }
                      onContextMenu={(event) =>
                        openSheetContextMenu(event, sheet)
                      }
                    ...
                    >
                      ...
                      >
                        ...
                        <div
                          className={`min-w-0 flex-1 ${compactSidebar ? "hidden" : ""}`}
                        >
                          {isRenaming ? (
                            <form
                            ...
                            >
                              <input
                                ...
                                onChange={(event) =>
                                  setRenamingSheetName(event.target.value)
                                }
                                onKeyDown={(event) => {
                                  if (event.key === "Escape") {
                                    event.preventDefault();
                ...
                {filteredSheets.length === 0 && (
                  <div
                    className={`rounded-2xl border border-dashed border-slate-300 bg-slate-50 text-center text-sm text-slate-500 ${compactSidebar ? "flex h-14 w-14 items-center justify-center p-0" : "px-4 py-8"}`}
                  >
                    {compactSidebar ? (
          ...
          <main className="min-w-0 flex-1 overflow-hidden">
            {activeSheet ? (
              <UniverSheetEditor
                ...
                workbookSheets={sheets.map((item) => ({
                  id: item.id,
                  name: item.name,
                }))}
                sheet={activeSheet}
              ...
              <div className="flex h-full items-center justify-center bg-slate-50 text-center">
                <div className="max-w-sm">
                  ...
                  <button
                    ...
                    disabled={!canManageWorkbook}
                    className="mt-6 inline-flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    <Plus className="h-4 w-4" />
                    新建工作表
                  </button>
            ...
            {sheetContextMenu.batch && selectedSheets.length > 1 ? (
              <>
                <div className="border-b border-slate-100 px-3 py-2">
                  <div className="text-xs font-semibold text-amber-800">
                    已选择 {selectedSheets.length} 个工作表
                  </div>
                  <div className="mt-0.5 truncate text-[11px] text-slate-400">
                    {selectedSheets.map((sheet) => sheet.name).join("、")}
                  </div>
                </div>
                {isAdminUser && (
                  <>
                    <button
                      type="button"
                      onClick={() => void handleBatchUpdateSheetState("lock")}
                      disabled={sheetActionLoading}
                      className="flex h-9 w-full items-center gap-2.5 px-3 text-left text-sm text-slate-700 hover:bg-amber-50 disabled:opacity-50"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => void handleBatchUpdateSheetState("unlock")}
                      disabled={sheetActionLoading}
                      className="flex h-9 w-full items-center gap-2.5 px-3 text-left text-sm text-slate-700 hover:bg-slate-50 disabled:opacity-50"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        void handleBatchUpdateSheetState("archive")
                      }
                      disabled={sheetActionLoading}
                      className="flex h-9 w-full items-center gap-2.5 border-t border-slate-100 px-3 text-left text-sm text-slate-700 hover:bg-slate-50 disabled:opacity-50"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        void handleBatchUpdateSheetState("unarchive")
                      }
                      disabled={sheetActionLoading}
                      className="flex h-9 w-full items-center gap-2.5 px-3 text-left text-sm text-slate-700 hover:bg-slate-50 disabled:opacity-50"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => void handleBatchUpdateSheetState("hide")}
                      disabled={sheetActionLoading}
                      className="flex h-9 w-full items-center gap-2.5 border-t border-slate-100 px-3 text-left text-sm text-slate-700 hover:bg-slate-50 disabled:opacity-50"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => void handleBatchUpdateSheetState("unhide")}
                      disabled={sheetActionLoading}
                      className="flex h-9 w-full items-center gap-2.5 px-3 text-left text-sm text-slate-700 hover:bg-slate-50 disabled:opacity-50"
                    >
                ...
                {canManageWorkbook && (
                  <button
                    type="button"
                    onClick={() => void handleBatchDeleteSheets()}
                    disabled={sheetActionLoading}
                    className="flex h-9 w-full items-center gap-2.5 border-t border-slate-100 px-3 text-left text-sm text-rose-600 hover:bg-rose-50 disabled:opacity-50"
                  >
                    <Trash2 className="h-4 w-4" />
                    批量删除
                  </button>
                ...
                <button
                  ...
                  className="flex h-9 w-full items-center gap-2.5 border-t border-slate-100 px-3 text-left text-sm text-slate-500 hover:bg-slate-50"
                >
                  <X className="h-4 w-4" />
                  清除选择
                </button>
              ...
              <>
                <button
                  ...
                  onClick={() => {
                    router.push(
                      `/sheets/${workbookId}/${sheetContextMenu.sheet.id}`,
                    );
                    setSelectedSheetIds([]);
                    setSheetContextMenu(null);
                  }}
                  className="flex h-9 w-full items-center gap-2.5 px-3 text-left text-sm text-slate-700 hover:bg-slate-50"
                >
                ...
                {canManageWorkbook && (
                  <button
                    type="button"
                    onClick={() =>
                      void handleDuplicateSheet(sheetContextMenu.sheet)
                    }
                    disabled={sheetActionLoading}
                    className="flex h-9 w-full items-center gap-2.5 px-3 text-left text-sm text-slate-700 hover:bg-sky-50 disabled:opacity-50"
                  >
                ...
                <button
                  type="button"
                  onClick={() => {
                    const target = sheetContextMenu.sheet;
                    setSheetContextMenu(null);
                    setWhatsAppResource({
                      sheetId: target.id,
                      title: `${workbook.name} / ${target.name}`,
                      defaultContent: `工作表：${workbook.name} / ${target.name}`,
                    });
                  }}
                  className="flex h-9 w-full items-center gap-2.5 px-3 text-left text-sm text-emerald-700 hover:bg-emerald-50"
                >
                ...
                {canManageWorkbook && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      startRenameSheet(sheetContextMenu.sheet);
                      setSelectedSheetIds([]);
                      setSheetContextMenu(null);
                    }}
                    className="flex h-9 w-full items-center gap-2.5 px-3 text-left text-sm text-slate-700 hover:bg-slate-50"
                  ...
                  <>
                    <button
                      ...
                      onClick={() => {
                        const target = sheetContextMenu.sheet;
                        setSheetContextMenu(null);
                        void handleUpdateSheetState(
                          target,
                          target.is_locked ? "unlock" : "lock",
                        );
                      }}
                      className="flex h-9 w-full items-center gap-2.5 border-t border-slate-100 px-3 text-left text-sm text-slate-700 hover:bg-slate-50"
                    >
                      {sheetContextMenu.sheet.is_locked ? (
                        <Unlock className="h-4 w-4 text-slate-400" />
                      ) : (
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        void handleUpdateSheetState(
                          target,
                          target.is_archived ? "unarchive" : "archive",
                        );
                      }}
                      className="flex h-9 w-full items-center gap-2.5 px-3 text-left text-sm text-slate-700 hover:bg-slate-50"
                    >
                      {sheetContextMenu.sheet.is_archived ? (
                        <ArchiveRestore className="h-4 w-4 text-slate-400" />
                      ) : (
                        <Archive className="h-4 w-4 text-slate-400" />
                      )}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        void handleUpdateSheetState(
                          target,
                          target.is_hidden ? "unhide" : "hide",
                        );
                      }}
                      className="flex h-9 w-full items-center gap-2.5 px-3 text-left text-sm text-slate-700 hover:bg-slate-50"
                    >
                      {sheetContextMenu.sheet.is_hidden ? (
                        <Eye className="h-4 w-4 text-slate-400" />
                      ) : (
                        <EyeOff className="h-4 w-4 text-slate-400" />
                      )}
        ...
        {historyPanelOpen && activeSheet && (
          <SheetHistoryPanel
            sheetId={activeSheet.id}
            sheetName={activeSheet.name || "未命名工作表"}
            canManage={canManageWorkbook}
            onClose={() => setHistoryPanelOpen(false)}
            onBeforeMutation={prepareDataMutation}
            onRestored={handleSheetReload}
          />
        )}
        <WhatsAppSendDialog
