...
function inferColumnTypeFromStyle(
  ...
  if (!cell?.s || typeof cell.s !== 'object') return column?.type

  const styleRef = cell.s as Record<string, unknown>
  const pattern = styleRef?.n && typeof styleRef.n === 'object'
    ? String((styleRef.n as Record<string, unknown>).pattern || '').toLowerCase()
    : ''
...
export function deriveColumnsFromUniverSheet(
  ...
  return Array.from({ length: columnCount }, (_, index) => {
    ...
    return {
      ...
      type: inferColumnTypeFromStyle(existing, rowZeroCell) || existing?.type || 'text',
      width:
        columnData[index]?.w ||
        existing?.width ||
        140,
