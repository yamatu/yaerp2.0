...
import { ArchiveRestore, ArrowLeft, Clock3, FileSpreadsheet, Folder, RefreshCw, Trash2 } from 'lucide-react'
import Link from 'next/link'
import { useCallback, useEffect, useMemo, useState } from 'react'
import { AuthGuard } from '@/components/auth/AuthGuard'
import api from '@/lib/api'
import type { Folder as FolderResource, RecycleBinContents, Workbook } from '@/types'
...
export default function RecycleBinPage() {
  const [contents, setContents] = useState<RecycleBinContents>({ folders: [], workbooks: [], retention_days: 30 })
  ...
  const total = contents.folders.length + contents.workbooks.length
  ...
  }, [contents.folders, contents.workbooks, retentionDays])

  const runAction = async (kind: 'folders' | 'workbooks', id: number, action: 'restore' | 'delete', name: string) => {
    if (action === 'delete' && !window.confirm(`确定要永久删除「${name}」吗？此操作无法还原。`)) return
      ...
      <div className="min-h-screen bg-slate-100 p-3 md:p-5">
        <div className="mx-auto max-w-[1200px] space-y-3">
          ...
          <section className="grid gap-3 sm:grid-cols-3">
            <div className="rounded-lg border border-slate-200 bg-white px-4 py-4 shadow-sm"><div className="text-xs text-slate-500">待处理内容</div><div className="mt-1 text-2xl font-semibold text-slate-950">{total}</div></div>
            <div className="rounded-lg border border-slate-200 bg-white px-4 py-4 shadow-sm"><div className="text-xs text-slate-500">已删除目录</div><div className="mt-1 text-2xl font-semibold text-slate-950">{contents.folders.length}</div></div>
            <div className="rounded-lg border border-slate-200 bg-white px-4 py-4 shadow-sm"><div className="text-xs text-slate-500">最近到期</div><div className="mt-1 text-2xl font-semibold text-slate-950">{total > 0 ? `${oldestRemaining} 天` : '-'}</div></div>
          </section>
          ...
          {loading ? (
            ...
            <div className="flex h-64 flex-col items-center justify-center rounded-lg border border-slate-200 bg-white text-center shadow-sm"><Trash2 className="h-8 w-8 text-slate-300" /><div className="mt-3 text-sm font-semibold text-slate-700">回收站为空</div><div className="mt-1 text-xs text-slate-400">删除的工作簿和文件夹会出现在这里</div></div>
          ) : (
