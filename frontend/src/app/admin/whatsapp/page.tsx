        ...
        <div className="grid min-h-[620px] xl:grid-cols-[330px_390px_minmax(0,1fr)]">
          ...
          <section className="border-r border-slate-200 bg-white p-5">
            {selectedAccount ? <>
              ...
              <div className="mt-5 flex flex-wrap gap-2"><button type="button" onClick={() => void runAction('start')} disabled={!settings.enabled || Boolean(acting)} className="inline-flex h-9 items-center gap-2 rounded-lg bg-[#008069] px-4 text-sm font-semibold text-white disabled:opacity-40"><Smartphone className="h-4 w-4" />启动/绑定</button><button type="button" onClick={() => void runAction('restart')} disabled={!settings.enabled || Boolean(acting)} className="inline-flex h-9 items-center gap-2 rounded-lg border border-slate-200 px-3 text-sm text-slate-600 disabled:opacity-40"><RefreshCw className={`h-4 w-4 ${acting === 'restart' ? 'animate-spin' : ''}`} />重启</button><button type="button" onClick={() => void runAction('logout')} disabled={selectedAccount.status !== 'ready' || Boolean(acting)} className="inline-flex h-9 items-center gap-2 rounded-lg border border-rose-200 px-3 text-sm text-rose-600 disabled:opacity-40"><LogOut className="h-4 w-4" />退出</button></div>
