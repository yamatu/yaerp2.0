...
import type { ColumnDef, Department, PageData, PermissionMatrix, User, Workbook } from '@/types'
...
interface SheetOption {
  ...
  workbookId: number
  workbookName: string
  columns: Array<Pick<ColumnDef, 'key' | 'name'>>
}
...
export default function PermissionsPage() {
      ...
      const sheetItems = details.flatMap((response, index) =>
        response.code === 0 && response.data?.sheets
          ? response.data.sheets.map((sheet) => ({
              id: sheet.id,
              name: sheet.name,
              workbookId: workbookItems[index].id,
              workbookName: workbookItems[index].name,
              columns: Array.isArray(sheet.columns) ? sheet.columns.map((column) => ({ key: column.key, name: column.name || column.key })) : [],
            }))
          : []
      )
  ...
  useEffect(() => {
    if (selectedSheet?.columns.length && !selectedSheet.columns.some((column) => column.key === draftColumn)) {
      setDraftColumn(selectedSheet.columns[0].key)
    }
    if (selectedSheet?.columns.length && !selectedSheet.columns.some((column) => column.key === previewColumn)) {
      setPreviewColumn(selectedSheet.columns[0].key)
    }
  ...
  const describeRule = (rule: RangeRule) => {
    const columnName = selectedSheet?.columns.find((column) => column.key === rule.column_key)?.name || rule.column_key
    if (rule.row_index !== undefined && rule.column_key) return `单元格 ${columnName}${dataRowToDisplayRow(rule.row_index)}`
        ...
        <div className="space-y-3">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-[1fr_1fr_150px]">
              ...
              <label className="text-xs font-medium text-slate-600">列
                <select value={previewColumn} onChange={(event) => setPreviewColumn(event.target.value)} className="mt-1.5 h-10 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none focus:border-sky-300">
                  {selectedSheet?.columns.map((column) => <option key={column.key} value={column.key}>{column.name}</option>)}
                </select>
              </label>
          ...
          <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm md:p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div><h2 className="text-base font-semibold text-slate-950">精确区域权限</h2><p className="mt-1 text-xs text-slate-500">优先级：个人规则 &gt; 部门规则 &gt; 角色默认；复杂区域建议直接在表格中拖动框选并配置白名单。</p></div>
              {selectedSheet && <a href={`/sheets/${selectedSheet.workbookId}/${selectedSheet.id}`} className="inline-flex h-9 shrink-0 items-center justify-center gap-2 rounded-lg border border-sky-200 bg-sky-50 px-3 text-xs font-semibold text-sky-700 transition hover:bg-sky-100"><ExternalLink className="h-3.5 w-3.5" />在表格中可视化配置</a>}
            </div>
