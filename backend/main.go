...
func main() {
	...
	mailService.SetAttachmentStore(uploadService)
	...
	api.Use(middleware.AuthMiddleware(jwtUtil, rdb))
	{
		...
		api.GET("/mail/accounts", mailHandler.ListOwnAccounts)
		api.POST("/mail/accounts", mailHandler.CreateOwnAccount)
		api.POST("/mail/accounts/test", mailHandler.TestOwnAccount)
		api.PUT("/mail/accounts/:id", mailHandler.UpdateOwnAccount)
		api.POST("/mail/accounts/:id/activate", mailHandler.ActivateOwnAccount)
		api.DELETE("/mail/accounts/:id", mailHandler.DeleteOwnAccountByID)
		...
		api.GET("/mail/messages/:uid/attachments/:partId/link", mailHandler.AttachmentLink)
		...
		api.GET("/mail/signatures", mailHandler.ListSignatures)
		api.POST("/mail/signatures", mailHandler.SaveSignature)
		api.PUT("/mail/signatures/:id", mailHandler.UpdateSignature)
		api.DELETE("/mail/signatures/:id", mailHandler.DeleteSignature)
		...
		api.POST("/trade/orders/:id/ai-import/complete", tradeHandler.CompleteAIImport)
		...
		api.PUT("/trade/orders/:id/customer-quotes/:quoteId/pi-seller-profile", tradeHandler.UpdatePISellerProfile)
		...
		api.POST("/ai/erp/import/apply", aiHandler.ApplyERPOrderImport)
		...
		admin.Use(middleware.RequireAdmin(userRepo))
		{
			...
			admin.POST("/admin/ai/assistants/:id/test", aiHandler.TestAssistant)
