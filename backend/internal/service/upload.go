...
// StoreOrReuseFile stores generated bytes in object storage and returns the row
// plus its signed access URL. Identical content is reused instead of uploaded
// again, which keeps repeated imports (mail attachments) from piling up rows.
func (s *UploadService) StoreOrReuseFile(filename, contentType string, data []byte, userID int64) (*model.Attachment, string, error) {
	if len(data) == 0 {
		return nil, "", fmt.Errorf("empty file content")
	}
	contentHash := sha256.Sum256(data)
	hash := hex.EncodeToString(contentHash[:])
	if existing, err := s.attachmentRepo.GetByContentHash(hash); err == nil && existing != nil {
		// Only reuse the row when it would be served with the same media type,
		// otherwise the browser could refuse to render it inline.
		if sameBaseContentType(existing.MimeType, contentType) {
			if reader, openErr := s.minioClient.GetObject(context.Background(), existing.ObjectKey); openErr == nil {
				_ = reader.Close()
				return existing, s.attachmentAccessURL(existing), nil
			}
		}
	}
	return s.UploadBytes(filename, contentType, data, userID)
}

// sameBaseContentType compares media types while ignoring parameters such as
// charset and any surrounding whitespace or casing.
func sameBaseContentType(left, right string) bool {
	normalize := func(value string) string {
		base := strings.SplitN(value, ";", 2)[0]
		return strings.ToLower(strings.TrimSpace(base))
	}
	leftType, rightType := normalize(left), normalize(right)
	if leftType == rightType {
		return true
	}
	// An untyped row would be served as raw bytes, so it is never reused.
	return false
}
